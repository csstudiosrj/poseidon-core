// src/app/(dashboard)/projeto/[id]/ProjetoContent.tsx
"use client";
...
export default function ProjetoContent({ projeto }: { projeto: any }) {
  const fontes = projeto.projeto_fontes || [];
  const itens = projeto.itens_orcamentarios || [];
  const orcamentoTotal = fontes.reduce((soma: number, f: any) => soma + Number(f.valor_captacao), 0);

  return (
    <div className="min-h-screen bg-sea-950 p-6 md:p-10 space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/hub" className="p-2 rounded-lg hover:bg-white/5 transition-colors text-white/50 hover:text-white/70">
          <ArrowLeft size={18} />
        </Link>
        <div className="flex-1">
          <h1 className="text-xl font-bold text-white tracking-tight">{projeto.nome_projeto}</h1>
